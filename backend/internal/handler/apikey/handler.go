...
type Handler struct {
	service apikeysvc.Service
}

// NewHandler creates a new API key handler.
func NewHandler(service apikeysvc.Service) *Handler {
...
func writeError(c *fiber.Ctx, err error) error {
	...
	case errors.Is(err, repository.ErrCustomerNotFound):
		return api.NotFound(c, "customer not found")
	case errors.Is(err, apikeysvc.ErrAPIKeyAlreadyRevoked):
		return api.Conflict(c, err.Error())
	case errors.Is(err, apikeysvc.ErrAPIKeyRotationInProgress):
		return api.Conflict(c, err.Error())
	case errors.Is(err, apikeysvc.ErrAPIKeyLimitReached):
		return api.Forbidden(c, err.Error())
	case errors.Is(err, apikeysvc.ErrInvalidGracePeriod):
		return api.BadRequest(c, err.Error())
	case errors.Is(err, apikeysvc.ErrInvalidPermissions):
		return api.BadRequest(c, err.Error())
	default:
